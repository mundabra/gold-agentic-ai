...
| Area | What GOLD does |
|---|---|
| Read-only | For read-only apps, a guardrail before any model call; SQLGlot parsing that allows one `SELECT`; single-statement execution; a database login with SELECT rights and read-only transactions. |
| Actions | Tools that change things return signed proposals; only the person who asked can approve; each approval runs once; the write login can do that one thing only ([approvals](approvals.md)). |
...
| Audit | One JSON record per question (answered or blocked): app, question, agents, tools, exact SQL, rows, proposed actions, tokens, time, trace ID; and one per approval decision. |
| Quality | `gold eval` as a CI gate; `gold bench` and `scripts/aiperf.sh` for latency and cost. |

## You add

- [ ] **Sign-in:** choose `GOLD_AUTH_MODE` (`proxy` behind your sign-in proxy, or `oidc`), set `GOLD_REQUIRE_IDENTITY=true`, and set a strong `GOLD_IDENTITY_SECRET`. GOLD has no user accounts of its own.
- [ ] **Row-level policies for your data:** entitlements and one policy per sensitive table ([identity](identity.md#for-your-data)).
...
- [ ] **Column grants for your data:** grant GOLD's login only the columns it may read, in the same way as `deploy/postgres/04-reader-role.sh`.
...
- [ ] **Data egress review:** with a hosted model (stage 1), the schema, questions, definitions, tool results and query results are sent to that provider.
- [ ] **Only the apps you need:** `apps.enabled` in Helm (or `GOLD_APPS`), and remove the other apps' components.
...
- **Approvals are by the person who asked.** There is no second approver yet, and a rejected proposal stays approvable until it expires (`GOLD_APPROVAL_TTL_SECONDS`). Without sign-in (`GOLD_AUTH_MODE=none`) anyone who can reach the UI can approve ([approvals](approvals.md#limits)).
- **The Sales copilot's "today" is the sample data's last invoice date** (`crm_as_of()`), because the data is a snapshot. Point it at `current_date` for live data.
