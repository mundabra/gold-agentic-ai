...
| Quality | `gold eval` as a CI gate; `gold bench` for latency and cost. |
...
- [ ] **Audit storage:** each response's `calls` trace (agents, tools, SQL, rows returned) is the audit record. Persist it to your log or audit store.
...
- [ ] **Observability:** send gateway traces to your OpenTelemetry stack.
...
- **The answer text is written by the orchestrator model.** It quotes the SQL and definitions, but the authoritative record is the trace (`calls[].steps`), which shows the exact query that ran.
