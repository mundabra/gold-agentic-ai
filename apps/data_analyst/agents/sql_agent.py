...
If run_sql's result has "visible_to", add: "These figures cover only the data visible to <that user>."
...
def build_agent(servers: list[MCPServerStreamableHttp]) -> Agent:
    ...
    async def generate_sql(ctx: RunContextWrapper, question: str, definitions: str = "") -> str:
        ...
        """
        schema = await schema_text() if settings.SQL_INCLUDE_SCHEMA else ""
